# Versioning - MII IG Symptom v2027.0.0-ballot.rc1
...
All released FHIR artifacts in the package carry the same version as the guide and its package. An artifact may therefore receive a new version on release even when the artifact itself did not change. The computable metadata (version algorithm, versioning policy, package source and manifest parameters) is declared by this module's [ImplementationGuide resource](ImplementationGuide-mii-ig-symptom.md).
