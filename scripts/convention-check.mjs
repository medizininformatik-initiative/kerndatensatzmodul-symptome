 ...
 * MIGRATION EDIT (DE-first, D-1 2026-08-31): this module is German-default with
 * the English translation under input/translations/en/ — the two trees swap. */
export function scanOptionalPages(root) {
  const dirs = {
    de: join(root, "input", "pagecontent"),
    en: join(root, "input", "translations", "en", "pagecontent"),
  };
...
function scanDuplicateHeadings(root) {
  ...
  // page titles: DEFAULT language (German, DE-first) from sushi-config pages:,
  // the ENGLISH translation from the IG-level .po under input/translations/en.
  // (Variable names keep the template's en/de spelling; the ROLES are swapped —
  // titlesEn = default-tree titles, titlesDe = translated-tree titles.)
  const titlesEn = {};
