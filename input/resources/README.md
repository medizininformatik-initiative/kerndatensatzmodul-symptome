...
1. **Replace `symptom` inside the file** — it appears in the resource
   `id` (`mii-param-symptom-manifest`). A `{{…}}` left in a FHIR `id` is
   ...
   `input/translations/en/ImplementationGuide-mii-ig-symptom.po`, which
